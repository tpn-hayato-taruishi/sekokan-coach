...
- 冒頭に件名候補、本文、の順。`;
    case 'ctable':
      return `# タスク: 実務経験証明書（C票 / 実務経験用紙）の記載内容を作成
${examLabel}（工事種別: ${koushu}）の受験申込に添付する実務経験の記載内容を、各記入欄ごとに整形して出力する。
次の欄を「欄名: 記載内容」の形で埋める（入力に無い欄は「未入力（要記入）」と書き、何を確認すべきか一言添える）:
- 工事名
- 工事の種別（${koushu}）
- 発注者名（注文者）
- 施工場所
- 従事した期間（着工〜完了の年月。実務経験年数の算定根拠）
- 請負代金額
- あなたの立場・職務内容（施工管理を行った立場であることが分かる表現に）
- 工事の内容（施工した設備・作業の具体的内容。${koushu}であることが明確に伝わるように）
最後に「実務経験としての要点」を2-3行（なぜこれが${koushu}の施工管理の実務経験と言えるか）。
${SEIDO_NOTE}`;
    case 'essay':
...
- これは下書きであり、本人が実体験に即して加筆・修正する前提であることを冒頭に明記。`;
    case 'check':
      return `# タスク: 経験記述・実務経験の「適合チェック（自己採点）」
入力された工事情報・記述が、${examLabel}の第二次検定 経験記述および受験資格の観点で通用するかを採点する。
各項目を ○（満たす）/ △（要改善）/ ×（不足）で判定し、×・△には具体的な改善指示を付ける:
1. 工事概要の必須項目が揃うか（工事名・施工場所・工期・請負代金額・立場・工事内容）
2. ${koushu}の施工管理として妥当か（他職種・他工事種別の流用に見えないか）
3. 「立場」が施工管理を行ったことを示すか（単なる作業員・設計のみ等でないか）
4. 課題→対策→結果が具体的で因果が通っているか（数値・固有条件があるか）
5. 減点・不認定リスク（一般論すぎ/数値なし/テーマとのズレ/期間の整合）
最後に「現状の総評」と「最優先で直すべき3点」を挙げる。合否を保証する表現はしない。
${SEIDO_NOTE}`;
  }
}
