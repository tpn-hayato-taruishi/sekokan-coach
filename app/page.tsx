...
**❌ 誤答 (or 真実の選択肢) の解説**
正答以外の3つを **必ず一つずつ** 解説 (省略禁止):
- 選択肢A: ${isNeg ? 'なぜ正しいのか (= 不適当でない理由)' : 'なぜ誤っているのか'} — 根拠: (条文/公式)
- 選択肢B: 同上
- 選択肢C: 同上
...
スマホで読める範囲で、内容は妥協なく書いてください (1000-1800字目安)。`,
  ...
  const explainAllChoices = useCallback(() => {
    if (!current) return;
    const isNeg = /不適当|誤って|誤った|定められていない|該当しない|関係のない|含まれない|除かれて/.test(current.question);
    const choicesEnumerated = (current.choices || []).map((c, i) => `### 選択肢${i + 1} ${(i + 1) === current.correct_answer ? `(=正答 — ${isNeg ? '虚偽の文' : '真実の文'})` : '(=誤答)'}\n本文: ${c}`).join('\n\n');
    chatRef.current?.sendMessage(
...
**🔍 設問の極性**: ${isNeg ? '不適当系 (正答=虚偽)' : '肯定系 (正答=真実)'} を冒頭で再確認
...
- 判定: ○/×
- 根拠: (具体的に、条文/公式/原理)
- ${isNeg ? '実際の事実は何か' : 'なぜ間違いなのか'}
- 覚えるキーワード/数値: 〜〜
- 引っ掛けの書き換え: 「${'${元の文中の○○}'}」を「${'${△△}'}」にすれば極性反転
- 関連で問われる: 〜〜
...
具体性が命です。一般論は無価値。条文番号・固有名詞・数値を必ず書く。`,
  ...
  const generateSimilar = useCallback(() => {
    if (!current) return;
    chatRef.current?.sendMessage(
      `この問題と **同じ解法パターン** で **数値・条件だけ変えた類題を2問** 作ってください。解法パターンを身につけることが目的です。
...
## 絶対のルール (1つでも違反したら全文書き直し)

**A. 各パターンには必ず根拠を併記:**
1. 過去問サンプル何問で同じパターンが現れたか (例: 「例1,例3,例5の3問で正答に "B種接地"」)
2. 物理法則・法令の絶対基準なのか (例: 「三相交流の線間電圧 = √3 × 相電圧」)
3. 上記のいずれも示せない場合は **書かない**

**B. パターンを3種類にラベル付け:**
- 🌐 **普遍**: 物理法則・法令の絶対基準。問題に依らず必ず成立 (例: 三相交流に√3、B種接地は150/Ig、絶縁抵抗0.1MΩ)
- 📊 **頻出**: 同テーマ過去問サンプルの **3問以上** で正答/誤答に繰り返し出現 (具体的に「例X,例Y,例Z」と明示)
- ⚠ **今回固有**: 現在の問題1問だけの数値・固有名詞 → **これは書くな**

**C. 評価:**
- 1問の選択肢の数値 (5, 15 等) からテーマ一般のルールを引き出すのは禁止
- 「問題文と矛盾するものは誤答」「計算結果と一致しないものは誤答」のような **当たり前の話は書かない** (受験生は当然知っている)
- 抽出できるパターンが無い場合は **「このテーマは固定パターンが少ない。本番では問題ごとに計算/個別判断が必要」と正直に書く**。無理にひねり出さない。

## 出力フォーマット (各項目に必ずラベルと根拠)

【🎯 正答に出やすいパターン】 (最大4個、根拠が無ければ書かない)
・[ラベル] パターン → 理由 (根拠: 例X,例Y,例Z)
例: [🌐普遍] 「B種接地」を含む選択肢 → 変圧器中性点接地は法令でB種固定 (電技解釈第17条)
例: [📊頻出] 「立ち会い」を含む選択肢 → 過去問6問中4問で正答 (例1,3,4,6)

【❌ 誤答に頻出するNGパターン】 (最大4個、根拠が無ければ書かない)
・[ラベル] パターン → 理由 (根拠)
例: [🌐普遍] 「省略できる」 → 安全関連の措置は省略不可が原則
例: [📊頻出] 「○○の場合に限り」 → 過去問サンプル5問中3問で誤答に出現

【🔑 二択まで絞ったときの最終判断】
迷う2選択肢が出たときの判断基準 (1-2個、根拠付きで)。
例: [🌐普遍] より厳しい基準を選ぶ (電気法規の絶対安全側の原則)

【📌 このテーマで本当に覚えるべき定数・固有名詞】 (3-5個)
普遍ラベルの値のみ列挙。例: 「中性点接地=B種」「絶縁抵抗=0.1MΩ以上」

【まとめ】
1行で「このテーマは [パターンで絞れる/問題ごとの計算が必要] 」と総括する。

簡潔に。「具体例 + 根拠」のセットで書く。根拠なしの一般論は書かない。`,
    );
  }, [current, data]);
  ...
  const generateMnemonic = useCallback(() => {
    if (!current) return;
    const isNegative = /不適当|誤って|誤った|定められていない|該当しない|関係のない|含まれない|除かれて/.test(current.question);
    chatRef.current?.sendMessage(
      `この問題に出てくる用語・単位・数値を **意味を考えずに音とイメージだけで暗記** できる語呂合わせを作ってください。

## ⚠️ 設問極性チェック (必ず先に確認)
この問題は **${isNegative ? '「不適当 / 誤り / 定められていない」を選ぶ問題' : '「適当 / 正しい」を選ぶ問題'}** です。
${isNegative
  ? `**正答番号 ${current.correct_answer || '?'}番の選択肢は虚偽の文** です。
- 暗記対象は **訂正後の真実** + **他の3つ (正しい文)** から抽出する事実 にしてください。
- 正答番号の選択肢の文を **そのまま暗記** させたら逆効果。必ず「実際は○○」と訂正してから語呂を作る。`
  : `**正答番号 ${current.correct_answer || '?'}番の選択肢が真実の文** です。これと、誤答3つから派生する「正しい知識」を暗記対象にします。`}

## 現在の問題
${current.question.slice(0, 300)}
選択肢:
${(current.choices || []).map((c, i) => `  ${i + 1}. ${c.slice(0, 100)}${current.correct_answer === i + 1 ? (isNegative ? ' ← 正答 = **誤った文 (訂正必要)**' : ' ← 正答 = **正しい文**') : ''}`).join('\n')}

## 「語呂合わせ」の定義
受験生が **意味を理解しなくても、音やイメージだけで暗記できる** 短いフレーズ。
**英字略号やカタカナ用語を、日本語の意味のある言葉に "音変換" する** のが定番技法。

### 良い語呂合わせ (こういうのを作る)
- **光度 = cd (カンデラ)** → 「**光**は **コ**ードで決まる」 (cd→code→コード)
- **照度 = lx (ルクス)** → 「**シャ**ルル(=lx)が **照**らす」 (lx→ルクス→シャルル王、照らす=照度)
- **光束 = lm (ルーメン)** → 「**光**を **ルー**ム(=lm)に **束**ねる」 (lm→ルーム=部屋)
- **第二種電気工事士 = 600V以下** → 「**ろっぴゃく**で **にしゅ**(2種)になる」
- **接地工事 A種 = 10Ω以下** → 「**エース**(A種)は **テン**(10)で **おしまい**」
- **絶縁抵抗 0.1MΩ** → 「**ぜっ**たい **イチ**(1)割(0.1) で **キレ**ない」(絶対1割で切れない=絶縁)
- **B種接地 = 150/Ig** → 「**ビー**(B)で **イチゴーマル**(150)を **イグ**ザクト(Ig)で割る」
- **着工届 = 10日前** → 「**ジュッ**(10)日前に **チャッ**コー(着工)を **トド**ける」

### ダメな例 (これは語呂合わせではない、絶対に書くな)
- 「輝度は面積当たりの光束」 ← ただの定義説明
- 「光度は光の強さ」 ← 言い換えただけ
- 「ニーマルのニーマルはヨコスロット」 ← 質問の単語をカタカナにしただけで暗記の助けにならない
- 「ジェイアイエスは正解のカギ」 ← 無意味なスローガン
- 「縦はタテ、横はヨコ」 ← 同じことの言い換え
- 「125Vは低圧、250Vは高圧」 ← 事実説明であって "音/イメージで覚える" 仕組みが無い

## ⛔ この問題が語呂合わせに向かない場合の処理

以下の問題タイプは **語呂合わせを無理やり作るな**:
- 図記号(JIS、消防、JIS C 等)を選ぶ視覚問題
- 配線図/回路図の構成を選ぶ問題
- 写真/イラストの判別問題
- 長文の手順を比較する問題 (例: 工程管理の手順4ステップ)

このタイプの問題では、語呂合わせの代わりに以下を提示してください:
1. **覚え方の代替案**: 「この問題は形を覚える/手順を覚える/事実を覚えるしかない。語呂合わせには向かない」と正直に書く
2. **特徴比較表**: 4つの選択肢を比較する **特徴の対応表** を作る (例: 125V=平行2本縦、250V=平行2本横、接地付=丸ピン付加 等)
3. **イメージで覚える指針**: 形状/数値/手順を視覚的に頭に焼き付けるためのヒント

## 必須プロセス (順番に実行)

### ステップ1: この問題は語呂合わせに向くか判定
- 数値/略号/単位/期限/法令名/物質名 が答えなら → **向く** → ステップ2へ
- 図記号/配線図/写真/手順比較 が答えなら → **向かない** → ステップ3へ

### ステップ2 (向く場合): 語呂を作る
覚えるべき対象を3-5項目に絞って、各項目に語呂を付ける。

各項目は:
- **対象**: 何を覚えたいか (例: 「光度の単位 = cd」「絶縁抵抗 0.1 MΩ」)
- **語呂**: 5-15文字、対象の英字や数値を日本語の意味のある言葉に音変換
- **解説**: 語呂の各部が対象のどの部分に対応するか

### ステップ3 (向かない場合): 正直に伝える + 代替提示
- 冒頭で「**この問題は語呂合わせに向きません** (図記号/視覚的な選択肢/長文比較のため)」と書く
- 代わりに **特徴比較表** を作る (4選択肢を比較できる軸で並べる)
- 視覚的に頭に焼き付ける **イメージ補助** を1-2個提示

## 出力フォーマット

**【⚖ 適性判定】**
この問題は語呂合わせに [向く / 向かない] (理由: 答えが [数値/略号 / 図記号/視覚情報] のため)

**【🎵 語呂合わせ または ⛔ 代替手段】**

向く場合:
- 対象A: 「(語呂)」 — (語呂のどこが何に対応するか)
- 対象B: 「(語呂)」 — 同上
- (3-5項目)

向かない場合:
- 「この問題は (理由) のため語呂合わせを無理に作っても暗記の助けになりません。代わりに以下の特徴を覚えてください」
- **特徴比較表** (markdown table 形式、4軸ぐらいで4選択肢を比較)
- **視覚イメージ補助**: 「○○の形を△△に似ていると思って覚える」など

**【🃏 暗記カード】**
表 → 裏 の対応カードを3-4枚 (対応関係が明確なもののみ作る、無理な暗記カードは作らない)

**【⚠ 混同しやすい用語/形/数値の見分け方】**
試験本番で混同しやすいペアを抽出し、それぞれの **決定的な違い** を1行で。
- 例 (語呂式): 「光度(cd) vs 光束(lm): cd=コード=点、lm=ルーム=面」
- 例 (特徴式): 「125V平行2本=縦長、250V平行2本=横長」(視覚的差異を一文字で覚える)

# 禁止事項 (これに該当するなら書き直し)

❌ 問題文の単語をカタカナで読んだだけ (「ニーマルのニーマルはヨコスロット」のような出力は絶対書かない)
❌ 「JISは正解のカギ」「Aは大事」のような無意味なスローガン
❌ 「縦はタテ、横はヨコ」のようなトートロジー
❌ 「125Vは低圧、250Vは高圧」のような事実説明 (語呂ではない)
❌ 元の選択肢の文をそのまま暗記カードに転記 (要約・対応関係に変換すること)

# 自己チェック

出力前に確認:
1. 適性判定を最初に書いたか?
2. 向かない問題で無理やり語呂を作っていないか?
3. 語呂を作った場合、各語呂は **英字略号 or 数値を音変換** している (= 暗記対象の文字を含む) か?
4. 暗記カードの表/裏が **試験で実際に問われる対応** になっているか?
5. 出力に上記の "禁止事項" が混入していないか?`,
    );
  }, [current]);
...
1. (角度名) 過去○問中×回出題 (例: H30, R2, R4で同じ趣旨)
2. ...
