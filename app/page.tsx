...
良い語呂合わせの例 (こういうのを作る):
- **光度 = cd (カンデラ)** → 「**光**は **コ**ードで決まる」 (光=光度、コード=code/cd)
- **照度 = lx (ルクス)** → 「**シャ**ルル(=lx)が **照**らす」 (照度=照らす、lx=シャルル)
- **光束 = lm (ルーメン)** → 「**光**は **ルー**ム(=lm)に **束**ねる」
- **第二種電気工事士 = 600V以下** → 「**ろっぴゃく**で **にしゅ**(2種)になる」
- **接地工事 A種 = 10Ω以下** → 「**エース**(A種)は **テン**(10)で **おしまい**」
- **絶縁抵抗 0.1MΩ** → 「**ぜっ**たい **イチ**(1)割(0.1) で **キレ**ない (切れない=絶縁)」

ダメな例 (これは語呂合わせではない、書くな):
- 「輝度は面積当たりの光束」 ← ただの定義説明、暗記の助けにならない
- 「光度は光の強さ」 ← 同上

## 出力フォーマット

【🎵 語呂合わせ】
このテーマで覚えるべき3-5項目について、語感ベースで覚えられる短いフレーズを作る。

例:
・光度=cd: 「光はコード(code=cd)で決まる」
・光束=lm: 「光をルーム(lm)に束ねる」
・照度=lx: 「シャルル(lx)が照らす」
・輝度=cd/m²: 「カンデラ毎平米=きらめき(輝)」

各項目は (覚える対象) → (語呂フレーズ) の形で、5-15文字の覚えやすいフレーズ。

【🃏 暗記カード】
表 → 裏 の対応を3-4枚作る (用語/単位/数値の対応):
- 表: 光度 / 裏: cd (カンデラ)
- 表: 1 lx / 裏: 1 lm/m²
など、試験で問われる対応関係を簡潔に。

【⚠ 混同しやすい用語の見分け方】
よく混同するペアと、それぞれの語呂的な見分け方。
例:
・光度(cd) vs 光束(lm): 「光度はcd=コード=点光源、光束はlm=ルーム=面全体」

語呂は **意味を超えて音やイメージで覚えられるもの** だけ書く。説明的なものは禁止。`,
    );
  }, [current]);
