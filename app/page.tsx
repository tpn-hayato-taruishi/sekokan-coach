...
## 守るべきこと (これが守れないと無価値)
1. **絶対に問題固有の数値を一般論にしない**:
   - 「選択肢に "15" があれば誤答」「選択肢に "5" があれば正答」 はこの問題の固有値であり、テーマ一般のルールではない。書くな。
   - 「選択肢に "√3" があれば正答候補」 も、計算問題なら正答に √3 が含まれる確率が高いだけで「√3=正答」のルールではない。書くな。
2. 上記の **過去問サンプル6問の選択肢を見比べて**、複数問題で繰り返し正答/誤答に現れる **法令用語・固有名詞・公式の形** を抽出する。最低 **3問以上** で同じパターンが見られて初めて「ルール」として書ける。
3. 該当するルールが抽出できない場合は **「このテーマは固定パターンが見いだせない。問題ごとに計算/個別判断が必要」と正直に書く**。無理にでっち上げない。
4. 「問題文と合わない選択肢を選ぶ」「計算結果と一致しないものは誤答」のような **当たり前の話は絶対書かない**。
